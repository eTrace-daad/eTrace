...
class VetoIntervention extends StatelessWidget {
  const VetoIntervention({super.key});
  ...
  Widget build(BuildContext context) {
    return Theme(
      ...
      child: VetoInterventionScreen(),
    );
  }
}

class VetoInterventionScreen extends StatefulWidget {
  final String? code;
  final Map<String, dynamic>? owner;
  final Map<String, dynamic>? bovinData;
  final String? dateScan;

  const VetoInterventionScreen({
    Key? key,
    this.code,
    this.owner,
    this.bovinData,
  ...
  @override
  _VetoInterventionScreenState createState() => _VetoInterventionScreenState();

  // Factory pour récupérer les infos depuis GoRouter
  static VetoInterventionScreen fromGoRouter(BuildContext context) {
    final state = GoRouterState.of(context);
    final extra = state.extra as Map<String, dynamic>? ?? {};
    return VetoInterventionScreen(
...
class _VetoInterventionScreenState extends State<VetoInterventionScreen>
...
// Décès
  ...
  final List<Map<String, dynamic>> interventionTypes = [
    {
      'label': 'Diagnostic',
  ...
  // Symptômes pré-enregistrés pour diagnostic
  final List<String> symptomesList = [
    'Fièvre',
    'Toux',
    'Diarrhée',
    'Perte d\'appétit',
  ...
  // Médicaments pré-enregistrés
  final List<Map<String, dynamic>> medicamentsList = [
    {
      'nom': 'Oxytétracycline',
      'certifie': true,
      'posologie': '10 ml/jour',
      'duree': '5 jours',
    },
    {
      'nom': 'Ivermectine',
    ...
    },
    {
      'nom': 'Amoxicilline',
      'certifie': true,
      'posologie': '15 ml/jour',
      'duree': '7 jours',
    },
  ];

  // Vaccins pré-enregistrés
  final List<Map<String, dynamic>> vaccinsList = [
    {
      'nom': 'Fièvre aphteuse',
      'certifie': true,
      'dose': '2 ml',
  ...
  // Pour la sélection du type d'intervention
  String? selectedInterventionType;
  List<String> selectedSymptomes = [];
  List<Map<String, dynamic>> selectedMedicaments = [];
  List<Map<String, dynamic>> selectedVaccins = [];
  String? diagnosticObservation;
  String? traitementObservation;
  String? vaccinationObservation;

  // Date intervention (auto)
  ...
  void _submitForm() {
    // Blocage si code falsifié ou nouveau bovin
    if (isFalsifiedCode || isNewBovinDialogShown) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
          isFalsifiedCode
              ? "Impossible de déclarer une intervention sur un code falsifié."
              : "Impossible de déclarer une intervention sur un nouveau bovin. Veuillez l'enregistrer d'abord.",
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        backgroundColor: Colors.red,
      ));
    ...
    // Vérification du type d'intervention sélectionné
    if (selectedInterventionType == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text("Veuillez sélectionner le type d'intervention."),
        backgroundColor: Colors.red,
      ));
      return;
    }
    ...
    // Validation spécifique selon le type d'intervention
    bool valid = true;
    String errorMsg = "";

    if (selectedInterventionType == 'diagnostic') {
      if (selectedSymptomes.isEmpty) {
        valid = false;
        errorMsg = "Veuillez sélectionner au moins un symptôme observé.";
      }
      if (selectedMedicaments.isEmpty) {
        valid = false;
        errorMsg = "Veuillez sélectionner au moins un médicament prescrit.";
      }
    } else if (selectedInterventionType == 'vaccination') {
      if (selectedVaccins.isEmpty) {
        valid = false;
        errorMsg = "Veuillez sélectionner au moins un vaccin administré.";
      }
    } else if (selectedInterventionType == 'traitement') {
      if (selectedMedicaments.isEmpty) {
        valid = false;
        errorMsg = "Veuillez sélectionner au moins un médicament administré.";
      }
    }
    ...
    // Sauvegarde des champs du formulaire principal
    _formKey.currentState!.save();

    // Sauvegarde des observations complémentaires selon le type
    if (selectedInterventionType == 'diagnostic') {
      // diagnosticObservation déjà sauvegardé via onSaved
    } else if (selectedInterventionType == 'vaccination') {
      // vaccinationObservation déjà sauvegardé via onSaved
    } else if (selectedInterventionType == 'traitement') {
      // traitementObservation déjà sauvegardé via onSaved
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Row(
        children: [
          ...
          Flexible(
            child: Text(
              "Intervention vétérinaire enregistrée avec succès !",
              style: TextStyle(fontWeight: FontWeight.bold),
            ),
          ),
        ],
    ...
    // Ici, tu peux ajouter la logique d'enregistrement en base ou d'appel API

    // Optionnel : reset du formulaire après enregistrement
    setState(() {
      selectedInterventionType = null;
      selectedSymptomes.clear();
      selectedMedicaments.clear();
      selectedVaccins.clear();
      diagnosticObservation = null;
      traitementObservation = null;
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Ce QR code correspond à un nouvel animal non encore enregistré. Mais vous n'avez pas l'autorisation de l'enregistrer.",
                      style: GoogleFonts.nunito(
                        fontSize: 16,
                    ...
                    ElevatedButton.icon(
                      ...
                      icon: const Icon(Icons.report_gmailerrorred,
                          color: Colors.white),
                      label: const Text("Signaler ce nouveau bovin"),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: SoftAppBar(
        title: "Intervention vétérinaire",
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 16),
              Text(
                  "Faites des interventions, diagnostiquez et suivez les alertes de santé du bovin.",
                  style: TextStyle(
                ...
                children: [
                  Expanded(
                    child: GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          color: Colors.blue.shade50, //Color(0xff4a7b58),
                          borderRadius: BorderRadius.circular(12),
                        ),
              ...
              //if (!blockAll) ...[
              Text(
                "Type d'intervention vétérinaire",
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18),
              ),
              SizedBox(height: 8),
              AnimatedSwitcher(
                ...
                child: GridView.builder(
                  ...
                  itemCount: interventionTypes.length,
                  ...
                  itemBuilder: (context, idx) {
                    final event = interventionTypes[idx];
                    final isSelected =
                        selectedInterventionType == event['value'];
              ...
              AnimatedSwitcher(
                duration: Duration(milliseconds: 500),
                child: selectedInterventionType == null
                    ? SizedBox.shrink()
                    : Padding(
                        padding: const EdgeInsets.only(top: 16.0, bottom: 8),
                        child: _buildInterventionForm(),
                      ),
              ),
              _buildSubmitButton(),
            ],
        ...
        return FadeTransition(
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                spacing: 8,
                children: symptomesList.map((symptome) {
                  final selected = selectedSymptomes.contains(symptome);
                  return FilterChip(
                    ...
                    avatar: selected
                        ...
                        : null,
                    //label: Text(symptome, style: TextStyle(color: selected ? Colors.blue
                    label: Text(symptome),
                    selected: selected,
                    onSelected: (v) {
                      setState(() {
                        if (v) {
                          selectedSymptomes.add(symptome);
                        } else {
                          selectedSymptomes.remove(symptome);
                        }
                      });
                    },
              ...
              Text("Médicaments prescrits",
                  style: TextStyle(fontWeight: FontWeight.bold)),
              _buildMedicamentsSearch(),
              ...selectedMedicaments
                  .map((medoc) => _buildMedicamentCard(medoc)),
              SizedBox(height: 18),
              _buildMultilineField(
                label: "Observations complémentaires",
                onSaved: (v) => diagnosticObservation = v,
              ),
              SizedBox(height: 18),
              _buildVetoSignatureInfo(),
            ],
          ),
        );
      case 'vaccination':
        return FadeTransition(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 12),
              Text("Vaccins administrés",
                  style: TextStyle(fontWeight: FontWeight.bold)),
              _buildVaccinsSearch(),
              ...selectedVaccins.map((vaccin) => _buildVaccinCard(vaccin)),
              SizedBox(height: 18),
              _buildMultilineField(
                label: "Observations complémentaires",
                onSaved: (v) => vaccinationObservation = v,
              ),
              SizedBox(height: 18),
              _buildVetoSignatureInfo(),
            ],
        ...
        return FadeTransition(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildMultilineField(
                label: "Observations complémentaires",
                onSaved: (v) => traitementObservation = v,
              ),
              SizedBox(height: 18),
              _buildVetoSignatureInfo(),
            ],
  ...
  Widget _buildMedicamentsSearch() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        /*TextFormField(
        ...
        Wrap(
          spacing: 8,
          children: medicamentsList.map((medoc) {
            final selected = selectedMedicaments.contains(medoc);
            return FilterChip(
              backgroundColor: Colors.blue.shade50,
              showCheckmark: false,
              //checkmarkColor: Colors.white,
              ...
              avatar: selected
                  ? Icon(Icons.check_circle, color: Colors.blue.shade700)
                  : null,
              //label: Text(medoc['nom'], style: TextStyle(color: selected ?
              label: Wrap(
                children: [
                  Text(medoc['nom']),
                  SizedBox(width: 6),
              ...
              onSelected: (v) {
                setState(() {
                  if (v) {
                    selectedMedicaments.add(medoc);
                  } else {
                    selectedMedicaments.remove(medoc);
                  }
                });
              },
              selectedColor: Colors.blue.shade100,
  ...
  Widget _buildVaccinsSearch() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        /*TextFormField(
          decoration: InputDecoration(
            labelText: "Rechercher un vaccin",
            filled: true,
            prefixIcon: Icon(Icons.search, color: Color(0xff4B5563)),
            fillColor: Color(0xfff5f5f5),
            border: OutlineInputBorder(
        ...
        Wrap(
          spacing: 8,
          children: vaccinsList.map((vaccin) {
            final selected = selectedVaccins.contains(vaccin);
            return FilterChip(
              backgroundColor: Colors.blue.shade50,
              showCheckmark: false,
              //checkmarkColor: Colors.white,
              ...
              avatar: selected
                  ? Icon(Icons.check_circle, color: Colors.blue.shade700)
                  : null,
              //label: Text(vaccin['nom'], style: TextStyle(color: selected ?
              label: Row(
                children: [
                  Text(vaccin['nom']),
                  SizedBox(width: 6),
              ...
              onSelected: (v) {
                setState(() {
                  if (v) {
                    selectedVaccins.add(vaccin);
                  } else {
                    selectedVaccins.remove(vaccin);
                  }
                });
              },
              selectedColor: Colors.blue.shade100,
  ...
  Widget _buildMedicamentCard(Map<String, dynamic> medoc) {
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 6),
            Text("Posologie : ${medoc['posologie']}",
                style: TextStyle(color: Colors.blueGrey)),
            Text("Durée : ${medoc['duree']}",
                style: TextStyle(color: Colors.blueGrey)),
          ],
        ),
      ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 6),
            Text("Dose : ${vaccin['dose']}",
                style: TextStyle(color: Colors.blueGrey)),
            Text("Durée : ${vaccin['duree']}",
                style: TextStyle(color: Colors.blueGrey)),
          ],
        ),
      ),
  ...
  Widget _buildVetoSignatureInfo() {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.edit_document, color: Colors.blue, size: 28),
          SizedBox(width: 8),
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            Text("Vétérinaire agréé par MINEPIA",
        ...
        Center(
          child: Container(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text("Enregistrer l'intervention",
